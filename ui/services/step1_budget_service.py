...
def persist_weekly_artifacts(snapshot: dict) -> None:
    ...
    st.session_state[DISCRETIONARY_SPENDING_MONTHLY_DERIVED] = float(snapshot.get("discretionary_spending_monthly", 0.0) or 0.0)
    st.session_state[WEEKLY_SAVINGS_DERIVED] = float(snapshot.get("weekly_savings", 0.0) or 0.0)

    st.session_state[FIXED_ITEMS_ROWS_WEEKLY] = rows_with_weekly_from_fixed_df(st.session_state.get(STEP1_FIXED_BREAKDOWN_DF))
    build_variable_items_rows_weekly()
...
def build_snapshot() -> dict:
    ...
    income_period = str(st.session_state.get(STEP1_INCOME_PERIOD, "Monthly"))

...
def seed_buffer_goal_inputs(snapshot: dict, planning_horizon: int) -> None:
    if STEP2_BUFFER_GOAL_AMOUNT_INPUT not in st.session_state:
        st.session_state[STEP2_BUFFER_GOAL_AMOUNT_INPUT] = float(snapshot.get("short_term_goal_amount", 1000.0) or 1000.0)
...
def intent_target_triplet(intent: str, baseline_weekly: float, buffer_required_weekly_value: float) -> tuple[float, float, float]:
    multipliers = {
    ...
    return safe_target, recommended_target, ambitious_target


def resolve_suggested_target(intent: str, *, safe_target: float, recommended_target: float, ambitious_target: float, buffer_required_weekly_value: float) -> tuple[float, str, str]:
...
def initialize_step2_state(snapshot: dict) -> dict:
    ...
    if STEP2_UNCERTAINTY_PRESET not in st.session_state:
        ...
        st.session_state[STEP2_UNCERTAINTY_PRESET] = snapshot_uncertainty if snapshot_uncertainty in UNCERTAINTY_OPTIONS else UNCERTAINTY_OPTIONS[0]
    ...
    planning_horizon = int(st.session_state.get(STEP2_PLANNING_HORIZON_WEEKS, snapshot.get("planning_horizon_weeks", 12)) or 12)
...
def build_target_context(snapshot: dict, intent: str, planning_horizon: int) -> dict:
    ...
    suggested_target, suggested_label, suggested_feedback = resolve_suggested_target(
        intent,
...
def apply_target_seed_if_needed(*, baseline_weekly: float, recommended_target: float) -> None:
    ...
    if baseline_weekly > 0.0 and (not user_touched_target) and (
        current_target <= 0.0 or not bool(st.session_state.get(STEP2_TARGET_INITIALIZED, False)) or baseline_changed_since_seed
    ):
        st.session_state[STEP1_TARGET_WEEKLY_SAVINGS] = float(recommended_target)
        st.session_state[STEP2_SELECTED_PRESET] = "Recommended"
...
def build_target_patch_payload(
    ...
    *,
    ...
    payload = {
        STEP1_TARGET_WEEKLY_SAVINGS: round_target(value),
...
def build_feasibility_view_model(snapshot: dict, *, intent: str, planning_horizon: int, target_weekly: float) -> dict:
    ...
    discretionary_weekly = max(monthly_to_weekly(safe_float(snapshot.get("discretionary_spending_monthly", 0.0), 0.0)), 0.0)
...
def build_step2_snapshot_update(snapshot: dict, *, intent: str, planning_horizon: int, uncertainty_preset: str, random_run_nonce: int, feasibility: dict) -> dict:
